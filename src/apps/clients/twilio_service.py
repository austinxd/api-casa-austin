...
class TwilioOTPService:
    ...
    def _store_otp_code(self, phone_number, otp_code):
        """
        Almacena temporalmente el código OTP para verificación
        Usando cache de Django por simplicidad
        """
        from django.core.cache import cache
        cache_key = f"whatsapp_otp_{phone_number}"
        # Almacenar por 10 minutos
        cache.set(cache_key, otp_code, 600)
        logger.info(f"OTP almacenado temporalmente para {phone_number}")

    def _get_stored_otp_code(self, phone_number):
        """
        Recupera el código OTP almacenado
        """
        from django.core.cache import cache
        cache_key = f"whatsapp_otp_{phone_number}"
        return cache.get(cache_key)

    def _clear_stored_otp_code(self, phone_number):
        """
        Limpia el código OTP almacenado
        """
        from django.core.cache import cache
        cache_key = f"whatsapp_otp_{phone_number}"
