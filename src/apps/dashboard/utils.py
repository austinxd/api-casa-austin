...
def get_stadistics_period(fecha_actual, last_day):
    ...
    for p in Property.objects.exclude(deleted=True):
        # Filtrar reservas desde hoy hasta fin de mes
        ...
        # Continuar con los cálculos de dinero como estaban
        pagos_recibidos_propiedad_mes = 0
        for r in reservations_from_current_day:
            pagos_recibidos_propiedad_mes += r.advance_payment

        valor_propiedad_mes = reservations_from_current_day.aggregate(pagos=Sum('price_sol'))['pagos'] or 0
        profit_propiedad_mes_airbnb = ProfitPropertyAirBnb.objects.filter(
            property=p, month=fecha_actual.month, year=fecha_actual.year
        ).aggregate(profit=Sum('profit_sol'))['profit'] or 0
