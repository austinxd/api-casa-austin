...
class AIOrchestrator:
    ...
    def _try_guards(self, session, inbound_message):
        ...
        # G_FAQ — Preguntas frecuentes (mascotas, check-in, wifi, etc.)
        result = guards.try_faq(session, user_text)
        if result is not None:
            logger.info(
                f"Guard activo: {result['intent']} (sesión {session.id})"
            )
    ...
    def _build_property_context(self):
        ...
        # --- INFO_CASAS: al cliente le decimos solo capacity_max. El campo
        # capacity_sleep es interno (panel admin/dashboards) — exponerlo al
        # cliente revela detalles de cama que ahuyentan reservas. ---
        info_lines = []
        for p in props:
            name = p['name']
        ...
        # --- CLASIFICACION_POR_TAMANO: rangos automáticos por capacity_max ---
